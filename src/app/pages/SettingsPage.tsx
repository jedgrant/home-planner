...
import { useFamily, useFamilyMembers } from '@/features/auth/hooks/useFamilyQueries'
...
export function SettingsPage() {
  ...
  const { data: family, isLoading: familyLoading } = useFamily(user?.familyId ?? null)
  const { data: members, isLoading: membersLoading } = useFamilyMembers(user?.familyId ?? null)
  ...
  async function onRename(values: RenameValues) {
    ...
    await queryClient.invalidateQueries({ queryKey: ['family', user.familyId] })
    setRenameSaved(true)
  ...
  async function handleRemoveMember(targetUid: string) {
    ...
    await queryClient.invalidateQueries({ queryKey: ['familyMembers', user.familyId] })
    await queryClient.invalidateQueries({ queryKey: ['family', user.familyId] })
  }
