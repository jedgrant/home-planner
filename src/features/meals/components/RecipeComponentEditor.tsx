...
function ComponentCard({ component, stores, disabled, onRemove, onChange, onSave }: ComponentCardProps) {
...
export function RecipeComponentEditor({
  ...
  function handleAdd() {
    ...
    const updated: RecipeComponent[] = [
      ...components,
      { componentId: nanoid(), name: newName.trim(), ingredients: [], tasks: [] },
    ]
    onChange(updated)
    onSave(updated)
    setNewName('')
  ...
  function applyPatch(componentId: string, patch: Partial<RecipeComponent>) {
    return components.map((c) => (c.componentId === componentId ? { ...c, ...patch } : c))
  }
